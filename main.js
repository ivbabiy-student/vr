...
let spherePositions, sphereNormals;
...
spherePositions = createSphereSurface(0.2)
sphereNormals = createSphereSurface(0.2)
...
function draw() {
  ...
  let matAclTranslation = m4.translation(tSphere.x, tSphere.y, tSphere.z)
  ...
  gl.lineWidth(1);
  gl.clear(gl.DEPTH_BUFFER_BIT);
  gl.uniformMatrix4fv(iModelViewProjectionMatrix, false, m4.multiply(modelViewProjection, matAclTranslation));
  drawPrimitive(gl.TRIANGLE_STRIP, [1, 1, 0, 1], spherePositions, sphereNormals); // цвет
  drawPrimitive(gl.LINE_STRIP, [0, 1, 0, 1], spherePositions, sphereNormals); // линии
  if (panner) {
    panner.setPosition(tSphere.x, tSphere.y, tSphere.z)
  }
